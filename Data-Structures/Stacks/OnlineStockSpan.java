/*
1: Complete the next function in class StockSpanner which collects daily price quotes for some stock, and returns the span of that stock's price for the current day.
2: The span of the stock's price today is defined as the maximum number of consecutive days (starting from today and going backwards) for which the price of the stock was less than or equal to today's price.

For example, if the price of a stock over the next 7 days were [100, 80, 60, 70, 60, 75, 85], then the stock spans would be [1, 1, 1, 2, 1, 4, 6].
*/

import java.io.*;
import java.util.*;

public class Main {

    static class StockSpanner {
        
        static class Pair{
            int price;
            int index;
            
            Pair(int price, int index){
                this.price = price;
                this.index = index;
            }
        }
        
        Stack<Pair> st;
        int day = 0;

        public StockSpanner() {
            st = new Stack<>();
        }
        
        public int next(int price) {
            //day -> current day/index
            
            //remove all smaller stock prices on consecutivr left
            while(this.st.size() > 0 && st.peek().price <= price){
               st.pop();
            }
            
            int gi = -1;//index of day having price greater
            if(st.size() > 0){//greater day exist
                gi = st.peek().index;
            }
            
            int span = day - gi;
                    
            st.push(new Pair(price, day));//push todays price with index
            
            day++;
            
            return span;
        }
    }

    public static void main(String[] args) throws Exception {
        BufferedReader read = new BufferedReader(new InputStreamReader(System.in));

        StockSpanner obj = new StockSpanner();

        while(read.ready()){
            int price = Integer.parseInt(read.readLine());
            System.out.println(obj.next(price));
        }        
    }
}
