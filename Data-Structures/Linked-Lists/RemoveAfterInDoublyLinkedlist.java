...
class Main {
  ...
  public static class DoublyLinkedList {
    ...
    public Node removeNodeAfter(Node refNode){
        Node nextNode = refNode.next;// not null always -> check before
        Node next2Node = nextNode.next;
        
        if(next2Node == null){//remove tail
            return removeLastNode();
        }else{
            nextNode.next = nextNode.prev = null;//cancel link
        
            //link
            refNode.next = next2Node;
            next2Node.prev = refNode;
    ...
    public int removeAfter(Node refNode) {
      // no node exist after refNode
      if(refNode.next == null){
            System.out.println("LocationIsInvalid: ");
            return -1;
      }
        
      Node node = removeNodeAfter(refNode);
      return node.data;
    }
