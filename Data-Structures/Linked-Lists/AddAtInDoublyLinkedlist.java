...
class Main {
  ...
  public static class DoublyLinkedList {
    ...
    public void addNodeAt(int index, Node node){
        if(index == 0) addFirstNode(node);//first
        else if(index == this.size) addLastNode(node);//last
        else{
            Node forw = getNodeAt(index);
            Node prev = forw.prev;
            
            //fit node between prev & forw
            
            prev.next = node;//with previous
            node.prev = prev;
            
            node.next = forw;//with forw
            forw.prev = node;
