...
public class Main {
  ...
  public static int maxPathSum_(TreeNode root){
    ...
    int lsum = maxPathSum_(root.left);//max sum in left subtree 
    int rsum = maxPathSum_(root.right);//max sum in right subtree
    ...
    //max sum from any of subtree
    int maxChildSum = Math.max(lsum, rsum);
    
    //if child subtrees gives -ve sum -> no need to take it in path sum 
    //maxChildSum < 0
    //else 0
