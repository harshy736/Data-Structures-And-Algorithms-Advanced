...
public class Main {
    ...
    private static TreeNode helper(int[] pre, int[] post, int preStart, int preEnd, int postStart, int postEnd,Map<Integer, Integer> map) {
        
        if (preStart > preEnd)//base
            return null;
            
        TreeNode root = new TreeNode(pre[preStart]);
        
        if (preStart == preEnd)//last node
            return root;
            
        int idx = map.get(pre[preStart + 1]);//position of left child to root in the post
        //elemnets left to idx -> children of left child of root -> left subtree
        //(idx - postStart + 1) -> no of elements in the left subtree
